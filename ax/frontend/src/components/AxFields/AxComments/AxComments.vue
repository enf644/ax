<template>
  <div>
    ...
    <div class='all-messages' v-if='this.rowGuid && this.currentUserShortName'>
      <div :style='{ height: this.maxHeight + "px" }' class='scrollBox' ref='scrollBox'>
        <div :key='message.guid' v-for='message in messages'>
          ...
          <div class='self-message-wrapper' v-if='isSelfMessage(message)'>
            <div class='self-message-body'>
              <div class='self-speech-bubble'>{{ message.text }}</div>
              <div>
                <i class='user-avatar fas fa-user-circle'></i>
              </div>
            </div>
    ...
    getMessages() {
      ...
      apolloClient
        .query({
          query: LOAD_MESSAGES,
          variables: {
            threadGuid: this.value,
            updateTime: Date.now()
          }
        })
        .then(data => {
          if (data.data.threadMessages) {
